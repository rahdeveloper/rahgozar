...
object AppConfig {
    ...
    /**
     * Which preset the current ruleset came from, or -1 when it came from
     * nowhere the app can name.
     *
     * Applying a preset overwrites every rule, so afterwards the rules
     * themselves no longer say which preset produced them. Without this the
     * routing screen can list the presets but cannot mark one, and the user has
     * no way to tell what they are already on.
     */
    const val PREF_ROUTING_PRESET_INDEX = "pref_routing_preset_index"
    ...
    /** Version of the app whose bundled geo assets were last unpacked. */
    const val PREF_ASSET_VERSION = "pref_asset_version"
    ...
    /**
     * The hidden profiles the panel's Smart tunnel candidates are stored
     * under: `panel-smart-0`, `panel-smart-1`, … in the panel's own order.
     *
     * They never enter the visible server list: they exist so ad traffic can
     * leave through a working path before AdMob is asked for a fill, and the
     * user is not supposed to see, pick or keep them. The panel's own ids are
     * UUIDs, so these names cannot collide with a real server.
     */
    const val SMART_PROFILE_PREFIX = "panel-smart-"

    /** The single-profile guid older builds stored; removed on every apply. */
    const val LEGACY_SMART_PROFILE_GUID = "panel-smart"

    /**
     * Whether this run is the ad flow's tunnel rather than the user's own.
     *
     * The two are built by the same pipeline but must not be routed the same
     * way: the user's tunnel honours their routing preset, while this one
     * exists solely to carry ad traffic and must carry *all* of it.
     */
    fun isSmartProfile(guid: String): Boolean =
        guid.startsWith(SMART_PROFILE_PREFIX) || guid == LEGACY_SMART_PROFILE_GUID
     ...
     *
     * Everything that pointed at the upstream project is gone: its repository,
     * issue tracker, wiki, Telegram channel, privacy policy, a base64-hidden
     * promotional link, its GitHub-hosted package-name list and its GitHub-hosted
     * geo data. Shipping those in a rebranded app sends our users — and their
     * request logs — to someone else's infrastructure.
     *
     * The About and legal links now come from the panel at runtime — see
     * [AppLinksManager]. Nothing user-facing is hardcoded here.
     */
    ...
    // Shipped in assets and copied out on first run — never downloaded. The
    // upstream release URL it used to be fetched from is gone with the rest of
    // the third-party endpoints.
    ...
    /**
     * The same measurement as [MSG_MEASURE_DELAY_SUCCESS], as a plain number.
     *
     * That one carries a translated sentence for display. Deciding whether a
     * server actually works by pattern-matching prose would break the first
     * time a translation changed, so the number travels separately: negative
     * means the tunnel is up but carries nothing.
     */
    ...
    /** One server's test has begun, so the list can mark that row alone. */
    const val MSG_MEASURE_CONFIG_TESTING = 75

    /**
     * Live throughput, service process → UI.
     *
     * The traffic counters live in the core, which runs in its own process, so
     * the UI cannot read them directly. The speed notification already samples
     * them on a timer; this rides along on that same sample rather than adding
     * a second one.
     *
     * Content is "upBytesPerSec,downBytesPerSec,sessionUpBytes,sessionDownBytes".
     */
