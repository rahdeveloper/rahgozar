...
object CoreConfigContextBuilder {
    ...
    fun build(context: Context, guid: String): CoreConfigContext? {
        ...
        // The ad tunnel takes none of the user's routing, and that has to
        // include the DNS built from it.
        //
        // Those rules do not only decide where a connection goes — a "direct"
        // domain rule also installs a domestic resolver scoped to exactly
        // those domains, whose queries then leave through the direct
        // outbound. With the default preset that covers Google's ad hosts, so
        // the lookups for them would go to 223.5.5.5 on the user's real
        // address even once the connections themselves were tunnelled. See
        // the matching guard in CoreConfigManager.configureRouting.
        val smart = AppConfig.isSmartProfile(guid)
        val routingResolvedOutbounds = if (smart) emptyList() else resolveRoutingOutbounds()
        ...
        val routingDomainRules = if (smart) emptyList() else collectRoutingDomainRulesForDns()
