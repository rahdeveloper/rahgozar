...
import com.rahgozar.app.dto.ConfigResult
import com.rahgozar.app.dto.CoreConfigContext
...
import com.rahgozar.app.enums.BalancerStrategyType
import com.rahgozar.app.enums.CoreResolvedType
...
import com.rahgozar.app.util.PackageUidResolver
...
object CoreConfigManager {
    ...
    /**
     * Keeps the core's traffic counters on.
     *
     * They used to be stripped whenever the speed *notification* preference was
     * off, which is the default. The home screen shows throughput as a
     * permanent part of its design, so without these it reported zero forever
     * and nothing said why. The preference still exists — it decides whether a
     * notification carries the numbers — but it no longer decides whether the
     * numbers exist.
     */
    private fun applySpeedDisabled(v2rayConfig: V2rayConfig) {
        // Deliberately empty: stats and policy always stay.
    }
    ...
    */
    ...
    private fun resolveOutboundDomainsToHosts(v2rayConfig: V2rayConfig) {
        ...
        for (item in proxyOutboundList) {
            ...
            if (newHosts.containsKey(domain)) {
                ...
                // A user-supplied hosts entry decides what the core dials, so
                // it is what the tun's exclusions have to be built from.
                CoreRoutePin.remember(domain, hostsEntryAddresses(newHosts[domain]))
                continue
            }

            // While this app rides the running tun, the remembered pin is the
            // only usable answer: the tun excluded exactly those addresses,
            // and a fresh lookup — besides going through a resolver that may
            // not be up yet — could name IPs the exclusions never covered.
            val insidePin = if (CoreRoutePin.insideTun) CoreRoutePin.recall(domain) else emptyList()
            val resolvedIps = insidePin.ifEmpty {
                HttpUtil.resolveHostToIP(domain, preferIpv6).orEmpty()
            }
            if (resolvedIps.isEmpty()) {
                continue
            }
            CoreRoutePin.remember(domain, resolvedIps)
    ...
    /** The address list a `dns.hosts` value holds — one string, or several. */
    private fun hostsEntryAddresses(entry: Any?): List<String> = when (entry) {
        is String -> listOf(entry)
        is List<*> -> entry.filterIsInstance<String>()
        else -> emptyList()
    }
    ...
    private fun configureRouting(
        ...
        // The ad tunnel does not take the user's routing rules.
        //
        // It would look harmless — the presets only send Iran, Russia, China
        // and the LAN direct — but geosite's CN category carries Google's own
        // ad hosts (doubleclick.net, googlesyndication.com, googleadservices
        // .com, gstatic.com, app-measurement.com among them), and the default
        // preset sends that category direct. With sniffing on, the ad request
        // would match by SNI and leave outside the tunnel that was brought up
        // to carry it. So this profile gets one rule — keep the LAN local —
        // and everything else goes to the proxy.
        if (AppConfig.isSmartProfile(configContext.guid)) {
            appendRoutingUserRule(configContext, smartLanRule(), v2rayConfig, policyGroupBalancerTags)
            return
        }
    ...
    /**
     * LAN stays local; the ad tunnel proxies everything else.
     *
     * Written as literal ranges rather than `geoip:private`, so this profile
     * needs no geo data files at all. The splash starts this tunnel on the
     * very first launch, where a `geoip:` reference the core cannot resolve
     * yet is the difference between a tunnel and a failed start.
     */
    private fun smartLanRule() = RulesetItem(
        remarks = "Smart: keep the LAN local",
        outboundTag = AppConfig.TAG_DIRECT,
        ip = arrayListOf(
