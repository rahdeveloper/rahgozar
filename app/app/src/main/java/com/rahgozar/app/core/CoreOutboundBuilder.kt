...
import com.rahgozar.app.dto.V2rayConfig.OutboundBean
...
import com.rahgozar.app.extension.nullIfBlank
...
//                    }
    ...
    private fun getServerAddress(profileItem: ProfileItem): String {
        ...
        // Inside our own tun, the core is given the address and never the name.
        //
        // A name would have to be looked up, and the lookup leaves this app's
        // uid — which is inside the tunnel now — so it goes into the tun, which
        // is waiting for this very dial to carry it. The two wait for each
        // other until the resolver gives up ten seconds later with "no such
        // host", and the tunnel never comes up. Handing over the pinned
        // address removes the lookup entirely; it is also, by construction,
        // the address the tun excluded, so the dial has somewhere to go.
        //
        // Nothing else changes: the TLS server name and any Host header come
        // from the profile, not from this field, so a CDN still sees the name
        // it routes on and the certificate still verifies against it.
        if (CoreRoutePin.insideTun) {
            CoreRoutePin.dialAddresses(domain).firstOrNull()?.let { return it }
            CoreRoutePin.dialAddresses(profileItem.server).firstOrNull()?.let { return it }
        }
