...
import com.rahgozar.app.dto.entities.AssetUrlCache
import com.rahgozar.app.dto.entities.AssetUrlItem
...
object MmkvManager {
    ...
    private const val KEY_RUN_GUID_OVERRIDE = "RUN_GUID_OVERRIDE"
    private const val KEY_ANG_CONFIGS = "ANG_CONFIGS"
    private const val KEY_SUB_SERVER_PREFIX = "SUB_SERVERS_"
    private const val KEY_SUB_IDS = "SUB_IDS"
    private const val KEY_WEBDAV_CONFIG = "WEBDAV_CONFIG"

    // Opened through [SecureStore], which encrypts the ones that carry
    // credentials — the full configurations and the server list — with a key
    // held in the Android Keystore. The rest are preferences and cache paths
    // and stay in the clear; see SecureStore.SENSITIVE_IDS for which is which.
    private val mainStorage by lazy { SecureStore.open(ID_MAIN) }
    private val profileFullStorage by lazy { SecureStore.open(ID_PROFILE_FULL_CONFIG) }
    private val serverRawStorage by lazy { SecureStore.open(ID_SERVER_RAW) }
    private val serverAffStorage by lazy { SecureStore.open(ID_SERVER_AFF) }
    private val subStorage by lazy { SecureStore.open(ID_SUB) }
    private val assetStorage by lazy { SecureStore.open(ID_ASSET) }
    private val settingsStorage by lazy { SecureStore.open(ID_SETTING) }
    ...
    /**
     * The server the next started core must run — the selection, unless a
     * one-off override is in place.
     *
     * The override exists for the Smart tunnel: the ad flow brings up a tunnel
     * of the panel's choosing without touching the user's selection, because
     * the selection is user state and every screen displays it. It lives in
     * multi-process storage because the daemon process is the reader, and it
     * is set and cleared only by the smart session (plus a reconcile pass at
     * launch, so a crash cannot leave the user's connect button starting the
     * wrong server).
     */
    fun getRunServer(): String? =
        mainStorage.decodeString(KEY_RUN_GUID_OVERRIDE)?.takeIf { it.isNotEmpty() }
            ?: getSelectServer()

    /** The override itself, for code that has to know whether one is set. */
    fun getRunGuidOverride(): String? =
        mainStorage.decodeString(KEY_RUN_GUID_OVERRIDE)?.takeIf { it.isNotEmpty() }

    fun setRunGuidOverride(guid: String?) {
        if (guid.isNullOrEmpty()) {
            mainStorage.remove(KEY_RUN_GUID_OVERRIDE)
        } else {
            mainStorage.encode(KEY_RUN_GUID_OVERRIDE, guid)
        }
    }
    ...
    fun encodeServerConfig(guid: String, config: ProfileItem): String {
        ...
        if (!serverList.contains(key)) {
            ...
            // Deliberately does NOT select this server. Storing a profile is not
            // choosing it, and picking whichever happened to be written first
            // means a dead server can end up selected before anything has
            // measured it. AutoPick in HomeViewModel makes that choice, from
            // latency, and only from servers that answered.
        }
