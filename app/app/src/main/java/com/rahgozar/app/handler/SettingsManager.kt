...
import com.rahgozar.app.AppConfig.GEOIP_PRIVATE
import com.rahgozar.app.AppConfig.GEOSITE_PRIVATE
import com.rahgozar.app.AppConfig.TAG_DIRECT
import com.rahgozar.app.AppConfig.VPN
import com.rahgozar.app.dto.V2rayConfig
import com.rahgozar.app.dto.entities.ProfileItem
import com.rahgozar.app.dto.entities.RulesetItem
import com.rahgozar.app.dto.entities.SubscriptionItem
import com.rahgozar.app.enums.EConfigType
import com.rahgozar.app.enums.Language
import com.rahgozar.app.enums.RoutingType
import com.rahgozar.app.enums.VpnInterfaceAddressConfig
import com.rahgozar.app.extension.moveItem
import com.rahgozar.app.handler.MmkvManager.decodeAllServerList
import com.rahgozar.app.handler.MmkvManager.decodeServerConfig
import com.rahgozar.app.handler.MmkvManager.decodeSubsList
import com.rahgozar.app.handler.MmkvManager.decodeSubscription
import com.rahgozar.app.handler.MmkvManager.encodeSubscription
import com.rahgozar.app.handler.MmkvManager.removeSubscription
...
object SettingsManager {
    ...
    /**
     * Puts the routing rules where the app requires them, on every start.
     *
     * There is one ruleset now — [RoutingType.WHITE_IRAN] — and no screen that
     * can change it. Iranian domains and IP ranges leave through the phone's
     * own connection; everything else goes through the tunnel.
     *
     * Written every start rather than only when the store is empty, which is
     * what it used to do. An install that has run an earlier build already has
     * a different preset saved, and with the routing screen gone there is
     * nothing left that could ever correct it — the seed-once version would
     * have left those devices on a ruleset the app no longer offers, forever.
     */
    private fun initRoutingRulesets(context: Context) {
        val rulesetList = getPresetRoutingRulesets(context, ROUTING_PRESET) ?: return
        MmkvManager.encodeRoutingRulesets(rulesetList)
        MmkvManager.encodeSettings(
            AppConfig.PREF_ROUTING_PRESET_INDEX, ROUTING_PRESET.toString(),
        )
    }

    /** [RoutingType.WHITE_IRAN], and the only one the app uses. */
    private const val ROUTING_PRESET = 3
    ...
    fun initAssets(context: Context, assets: AssetManager) {
        ...
        try {
            // Copied on a version change as well as when missing.
            //
            // These files ship inside the APK and are only ever written by
            // this method, so "already there" used to mean "nothing to do" —
            // which quietly froze whatever the first install unpacked. When
            // the geo data shipped with the app changes, an upgraded device
            // has to actually get it.
            val stamped = MmkvManager.decodeSettingsString(AppConfig.PREF_ASSET_VERSION)
            val current = BuildConfig.VERSION_CODE.toString()
            val refresh = stamped != current

            val geo = arrayOf(AppConfig.GEOSITE_DAT, AppConfig.GEOIP_DAT, AppConfig.GEOIP_ONLY_CN_PRIVATE_DAT)
            assets.list("")
                ?.filter { geo.contains(it) }
                ?.filter { refresh || !File(extFolder, it).exists() }
                ?.forEach {
            ...
            // Written only after the copies succeeded, so a run that failed
            // half way tries again rather than recording a lie.
            if (refresh) MmkvManager.encodeSettings(AppConfig.PREF_ASSET_VERSION, current)
        } catch (e: Exception) {
     ...
     */
    /**
     * The locale every string in the app is resolved against.
     *
     * Follows the FA/EN switch in the top bar, and nothing else. It used to
     * read a separate `pref_language` setting that the old settings screen
     * wrote — a screen that no longer exists, so nothing ever wrote it, so it
     * always fell through to the device language. The result was an app whose
     * screens were in Persian while its toasts and notifications were in
     * whatever the phone was set to.
     *
     * Only two languages exist here, which is why the ten-way mapping is gone
