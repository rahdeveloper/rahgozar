...
object Utils {
    ...
    fun isIpAddress(value: String?): Boolean {
        ...
        try {
            ...
            // Bracketed IPv6, with or without a port: [::1] and [::1]:80.
            //
            // The brackets exist precisely because a bare IPv6 address cannot be
            // told apart from host:port, so take what is inside them and discard
            // the rest. This used to be handled by isIpv6Address, but only for
            // the unported form — it stripped brackets only when the string both
            // began with '[' and ended with ']', which "[::1]:80" does not, so
            // every bracketed address carrying a port was rejected.
            //
            // Fixed here rather than in isIpv6Address on purpose: isPureIpAddress
            // also calls that one, and "pure" has to keep meaning an address with
            // no port attached.
            if (addr.startsWith("[")) {
     ...
     */
    fun isXray(): Boolean = BuildConfig.APPLICATION_ID.startsWith("com.rahgozar.app")
