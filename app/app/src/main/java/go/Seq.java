...
//
// RAHGOZAR: this is the one copy of the gomobile runtime in the app, shared by
// libv2ray (Xray) and libbox (sing-box). Both AARs shipped their own — the
// classes clash at build time, and worse, each Go runtime needs go.Seq's
// natives bound to *its* library, or refs from one core land in the other's
// ref table. One class can only bind one library per process, so the two
// cores are kept in separate processes (:RunSoLibV2RayDaemon vs
// :SingBoxDaemon, see AndroidManifest.xml) and this static block loads
// whichever runtime belongs to the process it finds itself in. The go/*
// classes are stripped from both AARs during import — see
// docs/SINGBOX-INTEGRATION.md.
public class Seq {
	...
	static {
		...
		// Every sing-box process — the tunnel's and the delay test's — gets
		// libbox; everything else gets Xray's. The names come from
		// AndroidManifest.xml and must stay in step with this check.
		if (process != null && process.contains(":SingBox")) {
	...
	// /proc/self/cmdline rather than Application.getProcessName(): this class
	// is initialised from static contexts on any API level from 24 up.
	private static String readProcessName() {
		try (java.io.FileReader reader = new java.io.FileReader("/proc/self/cmdline")) {
